...
input). Resize/crop choices are explicit design decisions, not something
SimA's README specifies precisely -- documented inline, reconsider if you have
a reason to prefer a different preprocessing convention.

SPLIT PROVENANCE NOTE: unlike CIFAR-10/100, no pre-made SecMI/SimA split file
exists for STL10-U or CelebA (SimA's Drive folder only has a CIFAR-10
checkpoint; no STL10-U/CelebA checkpoints or splits were found there). Splits
for these two datasets are therefore generated by this project's own
make_member_split() (reproducible, seeded), matching SimA's stated split
SIZES (50k/50k for STL10-U, 30k/30k for CelebA) but not any specific known
index assignment -- there is nothing external to match here.
