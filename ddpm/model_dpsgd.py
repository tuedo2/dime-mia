...
NOTE (DP-SGD compatibility): this is a SEPARATE file from ddpm/model.py, used
# ONLY by train_dpsgd.py -- the standard (non-DP) training/evaluation pipeline
# is untouched. The single in-place operation that existed in this file
(ResBlock.forward's `h += ...`) was changed to out-of-place (`h = h + ...`)
-- Opacus's per-sample gradient hooks (GradSampleModule) cannot handle
in-place modifications of tensors that flow through a custom autograd
Function, and raise "Output 0 of BackwardHookFunctionBackward is a view and
is being modified inplace" otherwise. This is a purely mechanical change --
identical forward-pass semantics, verified by inspection that no other
in-place op exists anywhere else in the file (all Sequential blocks,
attention's permute/view/cat, and the shortcut-residual add were already
